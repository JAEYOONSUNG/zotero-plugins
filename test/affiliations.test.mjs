...
test("tiers are cut points on the institution h-index, and a flag is two code points", () => {
	assert.equal(A.tierOf(1400).key, "exceptional");
	assert.equal(A.tierOf(1399).key, "high");
	assert.equal(A.tierOf(400).key, "established");
	assert.equal(A.tierOf(399), null);
...
test("the first author and the flagged corresponding author are picked out of a long list", () => {
	...
	assert.equal(where.first.tier, "high");
	...
	assert.equal(where.tier, "exceptional", "the better of the two labs sets the row's tier");
