...
test('captured installed baseline and current feature source/test mappings agree',()=>{
 ...
 const result=verifyInventory(read('features.json'),read('style-baseline.json'));assert.equal(result.baseline,50);assert.equal(result.additional,20);
});
