...
async function boot({failJCR=false,invalidJCR=false}={}){
 const prefs=new Map(),columns=new Map(),observers=new Map(),registered=new Set(),writes=[],errors=[],debug=[],loaded=[],attached=[],started=[];let next=1;
 const Z={initializationPromise:Promise.resolve(),DataDirectory:{dir:'/isolated/data'},Libraries:{userLibraryID:1,get:()=>({editable:true})},Reader:{_readers:[]},getMainWindows:()=>[{testWindow:true}],getMainWindow:()=>null,debug:value=>debug.push(value),logError:e=>errors.push(e),
  ...
  HTTP:{request:async(method,url)=>{assert.equal(method,'GET');const name=url.split('/').at(-1);assert.ok(['if-catalog.json','journal-registry.json','jcr-categories.json'].includes(name));
 ...
 const context=vm.createContext({Zotero:Z,console,URL,PathUtils:{join:(...p)=>p.join('/')},IOUtils:{exists:async()=>false,writeUTF8:async(...args)=>writes.push(args)},pref:(k,v)=>prefs.set(k,v)});
 ...
 return {Z,context,columns,observers,registered,writes,errors,debug,loaded,attached,started};
}
