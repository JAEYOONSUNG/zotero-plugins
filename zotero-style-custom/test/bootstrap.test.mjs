...
test('Gecko-like bootstrap loads every module without Node globals and keeps user UI untouched',async()=>{
 ...
 assert.equal(columns.size,19);assert.equal(Z.StyleCustom.Workbench.TABS.length,17);assert.equal(typeof Z.StyleCustom.readerTools.setVerticalTabs,'function');assert.equal(typeof Z.StyleCustom.libraryService.graph,'function');assert.equal(registered.size,1);assert.equal(writes.length,0);
