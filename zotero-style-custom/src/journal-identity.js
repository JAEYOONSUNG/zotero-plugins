...
(function (root) {
  ...
  let FLAT_ABBREVIATIONS = null;
  function abbreviate(title) {
    const name = text(title);
    if (!name) return "";
    const known = ABBREVIATIONS[name] || ABBREVIATIONS[name.replace(/^The /i, "")];
    if (known) return known;
    /* The table used to be walked and re-flattened on every call. Abbreviating
       the whole JCR catalog did that twenty-two thousand times and took two
       seconds; the flattened form is the same every time, so it is built once. */
    const flatKey = flat(name);
    if (!FLAT_ABBREVIATIONS) {
      FLAT_ABBREVIATIONS = new Map();
      for (const [full, short] of Object.entries(ABBREVIATIONS)) {
        const key = flat(full);
        if (!FLAT_ABBREVIATIONS.has(key)) FLAT_ABBREVIATIONS.set(key, short);
      }
    }
    const matched = FLAT_ABBREVIATIONS.get(flatKey);
